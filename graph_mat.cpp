 ...
 * utilizzeremo una matrice di adiacenza.
 * 
 * La matrice di adiacenza e' una struttura dati che rappresenta un grafo
 * tramite una matrice quadrata di dimensione V*V, dove V e' il numero di vertici
 * del grafo. Per ogni coppia di vertici (i, j) della matrice, se esiste un arco
 * che va da i a j, allora la cella (i, j) della matrice avra' un valore diverso
 * da infinito, altrimenti avra' un valore che corrisponde a True o al suo peso.
 * 
 * Esempio:
 ...
 * la lista di adiacenza, che e' un array di liste, dove ogni lista
 * rappresenta i nodi adiacenti ad un nodo. (adiacenti = raggiungibili)
 * 
 * Esempio:
    ...
    public:
        ...
        */
        Grafo(const size_t& n) {
            // Inizializza la matrice di adiacenza (se un nodo non e' raggiungibile avra' un peso infinito)
            this->adjacency_matrix = vector<vector<T>>(n, vector<T>(n, numeric_limits<T>::max()));
        }
        ...
        */
        void add_edge(const T& from, const T& to, const T& weight) {
            ...
            this->adjacency_matrix[to][from] = weight; // se lo vuoi bidirezionale togli questa riga
        }
        ...
        */
        vector<T> DFS(const T& src, const T& dst) {
            ...
            vector<T> path(this->adjacency_matrix.size(), numeric_limits<T>::max());
            vector<bool> visited(this->adjacency_matrix.size(), false);
            ...
            while(!to_visit.empty()) {
                ...
                for(size_t i = 0; i < this->adjacency_matrix[current].size(); i++) {
                    // Se il nodo non e' raggiungibile o e' stato visitato non ci interessa
                    if(this->adjacency_matrix[current][i] == numeric_limits<T>::max() || visited[i]) continue;

                    // Lo aggiungiamo alla lista dei nodi da visitare
                    to_visit.push(i);

                    // Memorizziamo il nodo precedente per poter ricostruire il percorso
                    // i -> nodo che stiamo per visitare da current
                    path[i] = current; 
                }
            }
        ...
        */
        pair<T, vector<T>> dijkstra(const T& src, const T& dst) {
            // path: contiene il nodo precedente di ogni nodo visitato
            vector<T> path(this->adjacency_matrix.size());
            vector<bool> visited(this->adjacency_matrix.size(), false);

            // costs: contiene il costo di ogni nodo visitato dal nodo di partenza
            vector<T> costs(this->adjacency_matrix.size(), numeric_limits<T>::max());
            ...
            */
            ...
            while(!priority_queue.empty()) {
                ...
                for(size_t adjacent_node = 0; adjacent_node < this->adjacency_matrix[current_node].size(); adjacent_node++) {
                    // Se il nodo non e' collegato, allora non lo consideriamo
                    if(this->adjacency_matrix[current_node][adjacent_node] == numeric_limits<T>::max()) continue;

                    T adjacent_cost = this->adjacency_matrix[current_node][adjacent_node];
...
int main() {
    ...
    cout << "DFS from 3 to 0: ";
    for(auto& p : grafo.DFS(3, 0)) {
        cout << p << " ";
    }
    cout << endl;

    cout << "BFS from 3 to 0: ";
    for(auto& p : grafo.BFS(3, 0)) {
        cout << p << " ";
    }
    cout << endl;

    auto[cost, path] = grafo.dijkstra(3, 0);
    cout << "Dijkstra from 3 to 0: ";
    for(auto& p : path) {
        cout << p << " ";
    }
    cout << "(cost: " << cost << ")" << endl;
